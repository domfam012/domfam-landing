...
gulp.task("watch", async function () {
	gulp.watch(
		[paths.src.scss.files, "!" + paths.src.custom.files, "!" + paths.src.icon.files, "!" + paths.src.bootstrap.typeFiles, "!" + paths.src.bootstrap.light, "!" + paths.src.bootstrap.variables],
...
gulp.task("copy:all", function (cb) {
	if (envConfig.run != "all") {
		...
		destPath = destPath.replace(paths.dist.base.dir, paths.dist.base.dir + "/" + envConfig.run);
		return gulp
			.src([paths.src.base.assets, "!" + paths.src.scss.files, "!" + paths.src.js.main, "!" + paths.src.js.files, "!" + paths.src.scss.dir, "!" + paths.src.images.files, "!" + paths.src.fonts.files])
		...
		return new Promise(function (resolve, reject) {
			for (let i = 0; i < envConfig.demos.length; i++) {
				...
				var proc = gulp.src([paths.src.base.assets, "!" + paths.src.scss.files, "!" + paths.src.js.main, "!" + paths.src.js.files, "!" + paths.src.scss.dir]).pipe(gulp.dest(destPath));
				if (i == envConfig.demos.length - 1) resolve(proc);
			}
		});
	}
